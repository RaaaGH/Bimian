        ...
        if (/shop|toko/i.test(command)) {
            ...
            case 'buy':
                switch (_type) {
                    ...
                    case 'diamond':
                        ...
                        break
                    case 'giok':
                            if (global.db.data.users[m.sender].money >= Bgiok * count) {
                                global.db.data.users[m.sender].giok += count * 1
                                global.db.data.users[m.sender].money -= Bgiok * count
                                conn.sendButton(m.chat, `Succes membeli 🛎️Giok dengan harga ${Bgiok * count} ${rpg.emoticon('money')}Money`, wm, 'inventory', usedPrefix + 'inv', m)
                            } else conn.send2Button(m.chat, `Uang anda tidak cukup untuk membeli ${count} 🛎️Giok dengan harga ${Bgiok * count} ${rpg.emoticon('money')}money`, wm, 'inventory', usedPrefix + 'inv', 'claim', usedPrefix + 'claim', m)
                        break
                    case 'common':
            ...
            switch (type) {
                ...
                case 'giok':
                        if (global.db.data.users[m.sender].money >= Bgiok * count) {
                            global.db.data.users[m.sender].giok += count * 1
                            global.db.data.users[m.sender].money -= Bgiok * count
                            conn.sendButton(m.chat, `Succes membeli ${count} 🛎️Giok dengan harga ${Bgiok * count} ${rpg.emoticon('money')}Money`, wm, 'inventory', usedPrefix + 'inv', m)
                        } else conn.send2Button(m.chat, `Uang anda tidak cukup untuk membeli ${count} 🛎️Giok dengan harga ${Bgiok * count} ${rpg.emoticon('money')}money`, wm, 'inventory', usedPrefix + 'inv', 'claim', usedPrefix + 'claim', m)
            ...
            switch (type) {
                ...
                case 'diamond':
                    ...
                    break
                case 'giok':
                    if (global.db.data.users[m.sender].giok >= count * 1) {
                        global.db.data.users[m.sender].giok -= count * 1
                        global.db.data.users[m.sender].money += Sdiamond * count
                        conn.reply(m.chat, `Succes menjual ${count} Giok, dan anda mendapatkan ${Sgiok * count} money`, m)
                    } else conn.reply(m.chat, `Diamond anda tidak cukup`, m)
                    break
                default:
                    return conn.send3Button(m.chat, Kchat, wm, `beli ${rpg.emoticon('potion')}potion`, usedPrefix + 'shop buy potion', `beli ${rpg.emoticon('limit')}limit`, usedPrefix + 'shop buy limit', `beli ${rpg.emoticon('armor')}armor`, usedPrefix + 'shop buy armor', m)
